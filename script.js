...
function render(){
  ...
  data.forEach((c,cIndex)=>{
    ...
    <div class="committee">
      <h3>
        <input value="${c.name}" oninput="updateName(${cIndex},this.value)">
      </h3>
    ...
    c.rows.forEach((r,rIndex)=>{
      ...
      overall += Number(r.amount || 0);

      html += `
      <tr>
        <td><input type="date" value="${r.date}"
          onchange="updateRow(${cIndex},${rIndex},'date',this.value)"></td>
        <td><input type="number" inputmode="numeric"
          value="${r.amount}"
          onchange="updateRow(${cIndex},${rIndex},'amount',this.value)"></td>
      </tr>`;
    });
    ...
    html += `
      ...
      <button class="add-row" onclick="addRow(${cIndex})">➕ Add Day</button>
    </div>`;
